...
import fi.oamk.chat.Message.Companion.from
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.value != null) {
                    val messagesFromDatabase =
                        (snapshot.value as HashMap<String, ArrayList<String>>)["messages"]
                    messages.clear()

                    if (messagesFromDatabase != null) {
                        for (i in 0 until messagesFromDatabase.size){
                            val message: Message = Message.from(messagesFromDatabase[i] as HashMap<String, String>)
